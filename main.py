...
class Group(db.Model):
    __tablename__ = 'group'
    id = db.Column(db.Integer, primary_key=True)
    group_name = db.Column(db.String(200), nullable=False, unique=True)
...
if __name__ == '__main__':
    with app.app_context():
        # db.drop_all()
        db.create_all()
        g1 = Group(group_name='g1' + str(random.randint(1, 1000)))
        name = 'Azaliya' + str(random.randint(1, 1000))
        p1 = User(name=name, group=g1)
        name = 'Ameliya' + str(random.randint(1, 1000))
        p2 = User(name=name)
        name = 'Ameliya' + str(random.randint(1, 1000))
        p3 = User(name=name)
        g2 = Group(group_name='g1' + str(random.randint(1, 1000)), users=[p2, p3])

        with db.session.begin():
            db.session.add(p1)
            db.session.add(g2)
        print(User.query.filter(User.name == 'Azaliya270').first().name)
        # db.session.commit()
